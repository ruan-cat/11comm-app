...
export default function useUpload<T extends TfileType>(options: TOptions<T> = {} as TOptions<T>) {
  ...
  const handleFileChoose = ({ tempFilePath, size }: { tempFilePath: string, size: number }) => {
    ...
    if (size > maxSize) {
      toast.warning(`文件大小不能超过 ${maxSize / 1024 / 1024}MB`)
      return
    }
  ...
  const run = () => {
    ...
    const chooseFileOptions = {
      ...
      success: (res: any) => {
        ...
        // h5中res:{errMsg: "chooseImage:ok", tempFilePaths: "blob:http://localhost:3000/f74ab6b8-a14d-4cb6-a10d-fcf4511a0de5", tempFiles: [File]}
