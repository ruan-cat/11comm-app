...
import { auditUndo, queryNextDealUser } from '@/api/oa-workflow'
import { useGlobalToast } from '@/hooks/useGlobalToast'

definePage({
  style: {
    navigationBarTitleText: '审核待办',
  },
})
...
const { send: loadNextTask } = useRequest(
  (params: { taskId: string, flowId: string, id: string, startUserId: string }) => queryNextDealUser(params),
  { immediate: false },
)
  .onSuccess((event) => {
...
onLoad((options) => {
  ...
  startUserId.value = options?.startUserId || ''
